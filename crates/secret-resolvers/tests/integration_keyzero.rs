use secret_resolvers::{KeyzeroResolver, ResolveRequest, SecretResolver};
...
async fn test_keyzero_resolve_first_secret() {
    ...
    assert_eq!(result.value, "super-secret-db-pass");
...
async fn test_keyzero_resolve_named_secret() {
    ...
    let result = resolver.resolve(&request).await.unwrap();
    assert_eq!(result.value, "api-key-value");
...
async fn test_keyzero_no_token_sends_no_auth() {
    ...
    let result = resolver.resolve(&request).await.unwrap();
    assert_eq!(result.value, "no-auth-value");
