use base64::Engine;
use secret_resolvers::{GcpResolver, ResolveRequest, SecretResolver};
...
async fn test_gcp_resolve_secret_short_name() {
    ...
    assert_eq!(result.value, "my-gcp-secret-value");
}
...
async fn test_gcp_resolve_secret_with_version() {
    ...
    let result = resolver.resolve(&request).await.unwrap();
    assert_eq!(result.value, "versioned-secret-value");
}
...
async fn test_gcp_resolve_full_resource_name() {
    ...
    let result = resolver.resolve(&request).await.unwrap();
    assert_eq!(result.value, "full-resource-secret");
}
