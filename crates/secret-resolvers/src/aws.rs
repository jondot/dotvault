        ...
        if let Some(secret_id) = reference.strip_prefix("sm://") {
            ...
            if let Some(field) = request.get_str_opt("field") {
                ...
                let value = parsed
                    ...
                    .to_string();
                return Ok(ResolvedSecret { value: SecretString::from(value), ttl: None });
            }

            Ok(ResolvedSecret { value: SecretString::from(raw.to_string()), ttl: None })
    ...
    async fn write(&self, request: &WriteRequest) -> Result<()> {
        ...
        if let Some(secret_id) = reference.strip_prefix("sm://") {
            ...
            let result = self
                ...
                .put_secret_value()
                .secret_id(secret_id)
                .secret_string(request.value.expose_secret())
                .send()
                .await;
